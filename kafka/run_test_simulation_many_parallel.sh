...
do
  ...
  do
    ...
    # no limit
    ./kafka-producer-perf-test.sh --topic ${TOPIC_NAME}${CURRENT}  --throughput ${V_THROUGHPUT} --num-records ${V_NUMRECORDS} --record-size ${V_RECORDSIZE} --producer-props acks=all bootstrap.servers=${BOOTSTRAP_SERVERS} | awk -v var="$CURRENT" '{print var": " $0 }' &
    sleep 1
